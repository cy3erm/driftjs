...
def test_sourcemap_parse_and_rescan():
    ...
    pairs = parse_sourcemap(sm)
    assert clean_source_path(pairs[0][0]) == "src/api/admin.js"
    merged = extract(pairs[0][1])
    merged.merge(extract(pairs[1][1]))
...
def test_sourcemap_handles_no_content():
    ...
    import json
    pairs = parse_sourcemap(json.dumps({"version": 3, "sources": ["a.js"]}))
    assert pairs == [("a.js", None)]


def test_sourcemap_preserves_index_alignment():
    from driftjs.sourcemap import parse_sourcemap
    import json
    sm = json.dumps({"sources": ["a.js", "", "b.js"],
                     "sourcesContent": ["AAA", "BBB", "CCC"]})
    pairs = parse_sourcemap(sm)
    assert ("a.js", "AAA") in pairs
    assert ("b.js", "CCC") in pairs
    assert all(src for src, _ in pairs)
...
# ---- real-target false-positive regressions ----

def test_react_filename_not_flagged():
    from driftjs.libraries import find_libraries
    assert not any(n == "React" for n, v, note in find_libraries("react-7.5.1.js react-dom.19.2.3.js"))


def test_local_property_access_not_internal_host():
    from driftjs.analyze import find_internal_hosts
    h = find_internal_hosts("this.local; dev.local; h.lan; obj.local")
    assert not any(x.endswith((".local", ".lan")) for x in h)


def test_url_local_host_still_caught():
    from driftjs.analyze import find_internal_hosts
    assert "printer.local" in find_internal_hosts('u="http://printer.local/x"')


def test_i18n_key_not_generic_secret():
    from driftjs.secrets import find_secrets
    assert find_secrets('const x = "a11y_widget_keyboard_nav_enabled_word";') == set()


# ---- watch mode ----

def test_watch_alert_formatting():
    from driftjs.watch import format_alert
    alert = format_alert("https://t.com", ["/api/new"], [("Stripe", "sk_...x")], [("high", "XSS")])
    assert "/api/new" in alert and "Stripe" in alert and "XSS" in alert


def test_watch_webhook_payload_shape(monkeypatch):
    import driftjs.watch as W
    ...
    W.notify_webhook("https://discord.com/api/webhooks/x", "hi")
    assert "content" in captured["body"]
    W.notify_webhook("https://hooks.slack.com/services/x", "hi")
    assert "text" in captured["body"]


def test_diff_tracks_new_weaknesses():
    from driftjs.extract import extract
    from driftjs.snapshot import diff
    a = extract("const x = 1;")
    b = extract("el.innerHTML = location.hash;")
    d = diff(a, b)
    assert any("DOM XSS" in label for sev, label in d.new_weaknesses)


# ---- HTML report ----

def test_html_report_contains_findings():
    from driftjs.report import render_report
    from driftjs.extract import extract
    stripe = "sk_" + "live_" + "4eC39HqLyjWDarjtT1zdp7dcXYZ0"
    ex = extract(f'fetch("/api/admin/x"); el.innerHTML=location.hash; k="{stripe}";')
    h = render_report("https://t.com", ex, {})
    assert "driftjs report" in h
    assert "/api/admin/x" in h
    assert "DOM XSS" in h
    assert "Stripe" in h
...
def test_html_report_escapes_html():
    ...
    from driftjs.extract import extract
    ex = extract('fetch("/x")')
    ex.comments.add("<script>alert(1)</script>")
    h = render_report("t", ex, {})
...
def test_osv_parse_vulns():
    from driftjs.cve import parse_vulns
    resp = {"vulns": [
        {"id": "GHSA-x", "aliases": ["CVE-2020-11022"], "summary": "XSS in jQuery",
         "database_specific": {"severity": "MEDIUM"}},
        {"id": "GHSA-y", "aliases": ["CVE-2019-11358"], "summary": "Prototype pollution"},
    ]}
    v = parse_vulns(resp)
    assert v[0][0] == "CVE-2020-11022" and v[0][2] == "MEDIUM"
    assert v[1][0] == "CVE-2019-11358"


def test_osv_lookup_unknown_lib_returns_empty():
    from driftjs.cve import lookup_cves
    assert lookup_cves("NotARealLibrary", "1.0.0") == []


def test_osv_lookup_mocked(monkeypatch):
    import driftjs.cve as C
    monkeypatch.setattr(C, "_post", lambda url, payload, timeout=20: {
        "vulns": [{"id": "GHSA-z", "aliases": ["CVE-2019-1234"], "summary": "test vuln"}]})
    v = C.lookup_cves("jQuery", "1.7.2")
    assert v and v[0][0] == "CVE-2019-1234"


# ---- spec fixes: json completeness, diff, only/exclude, since-last ----

def test_json_output_completeness():
    import driftjs.cli as cli
    from driftjs.extract import extract
    from driftjs.snapshot import diff, _DIFF_CATS
    ex = extract('fetch("/api/a?x=1"); el.innerHTML=location.hash')
    d = diff(None, ex)
    obj = cli._build_json("t", ex, d, {})
    for cat in _DIFF_CATS:
        assert f"new_{cat}" in obj
    assert "gone_endpoints" in obj and "gone_params" in obj
    import json
    json.dumps(obj)  # must serialize


def test_per_target_path():
    import driftjs.cli as cli
    assert cli._per_target_path("out.html", "https://api.example.com/") == "out.api.example.com.html"
    assert cli._per_target_path("w.txt", "http://x.io") == "w.x.io.txt"


def test_url_query_extraction():
    from driftjs.extract import extract
    ex = extract('fetch("https://ex.com/api/users?debug=true&role=admin")')
    assert "debug" in ex.params and "role" in ex.params


def test_template_endpoint_normalized():
    from driftjs.extract import extract
    ex = extract('fetch(`/api/users/${id}?debug=true`)')
    assert any("/api/users/{var}" in e for e in ex.endpoints)


def test_json_endpoint_retention():
    from driftjs.extract import _clean_endpoint
    assert _clean_endpoint("/swagger/v1/swagger.json") is not None
    assert _clean_endpoint("/api/users.json") is not None
    assert _clean_endpoint("/manifest.json") is None


def test_param_key_body_extraction():
    from driftjs.extract import extract
    ex = extract("const b = { email: x, password: y, userId: z };")
    assert {"email", "password", "userId"} <= ex.params


def test_subdomain_sibling_rejected():
    from driftjs.subdomains import _parse
    subs = _parse('[{"name_value":"api.example.com","common_name":"notexample.com"}]', "example.com")
    assert "api.example.com" in subs and "notexample.com" not in subs


def test_fetch_modern_discovery():
    from driftjs.fetch import _script_urls
    html = ('<script src="/a.JS"></script><link rel="modulepreload" href="/b.mjs">'
            '<script>import("/c.js")</script>')
    urls = _script_urls("https://x.com", html)
