...
_URL = re.compile(r"""['"`](https?://[a-zA-Z0-9._\-]+(?:/[a-zA-Z0-9_\-./]{0,120})?)['"`]""")

_CALL = re.compile(r"""(?:fetch|axios\.\w+|\.(?:get|post|put|patch|delete)|open)\s*\(\s*['"`]([^'"`]{1,160})['"`]""")

_PARAM = re.compile(r"""[?&]([a-zA-Z_][a-zA-Z0-9_]{0,40})=""")
_PARAM_KEY = re.compile(r"""['"`]([a-zA-Z_][a-zA-Z0-9_]{1,40})['"`]\s*:\s*""")


_ASSET = re.compile(r"\.(png|jpe?g|gif|svg|webp|ico|css|js|mjs|map|woff2?|ttf|eot|mp[34]|pdf|json)(\?|$)", re.I)
...
def _clean_endpoint(e: str) -> tuple[str, set[str]] | None:
    e = e.strip().split("#")[0]
    if not e or _NOISE.search(e) or _ASSET.search(e) or _THIRD_PARTY.search(e):
        return None
